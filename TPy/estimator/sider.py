...
class SIDeRSVM(BaseEstimator, TransformerMixin):
    ...
    def fit(self, Xl, y, Dl, Xu=None, Du=None):
        """
        solve min_x x^TPx + q^Tx, s.t. Gx<=h, Ax=b
        Parameters:
            Xl: Labelled training data, array-like, shape (nl_samples, n_feautres)
            y: Label, array-like, shape (nl_samples, )
            Dl: Domain covariate matrix for training data, array-like, shape (nl_samples, n_covariates)
            Xu: Unlabelled testing data, array-like, shape (nu_samples, n_feautres)
            Du: Domain covariate matrix for testing data, array-like, shape (nu_samples, n_covariates)
        """
        ...
        if self._lb.classes_.shape[0] == 2:
            self.coef_, self.support_ = semi_binary_dual(K, y_, Q_inv,
                                                         self.C, self.solver)
            self.support_vectors_ = Xl[self.support_]
            ...
            for i in range(y_.shape[1]):
                ...
                self.support_vectors_.append(Xl[support_][-1])
    ...
    def fit_predict(self, Xl, y, Dl, Xu=None, Du=None):
        """
        solve min_x x^TPx + q^Tx, s.t. Gx<=h, Ax=b
        Parameters:
            Xl: Labelled training data, array-like, shape (nl_samples, n_feautres)
            y: Label, array-like, shape (nl_samples, )
            Dl: Domain covariate matrix for labelled data, array-like, shape (nl_samples, n_covariates)
            Xu: Testing testing data, array-like, shape (nu_samples, n_feautres)
            Du: Domain covariate matrix for testing data, array-like, shape (nu_samples, n_covariates)
        Return:
            predicted labels, array-like, shape (nu_samples,)
        """
        self.fit(Xl, y, Dl, Xu, Du)
...
class SIDeRLS(BaseEstimator, TransformerMixin):
    ...
    def fit_predict(self, Xl, y, Dl, Xu=None, Du=None):
        """
        Parameters:
            Xl: Training data, array-like, shape (nl_samples, n_feautres)
            y: Label, array-like, shape (nl_samples, )
            Dl: Domain covariate matrix for training data, array-like, shape (nl_samples, n_covariates)
            Xu: Testing data, array-like, shape (nu_samples, n_feautres)
            Du: Domain covariate matrix for testing data, array-like, shape (nu_samples, n_covariates)
        Return:
            predicted labels, array-like, shape (nu_samples,)
        """
